package com.discordclone.notificationservice.listener;

import com.discordclone.common.event.EventEnvelope;
import com.discordclone.common.event.EventTypes;
import com.discordclone.common.event.MessageSentEvent;
import com.discordclone.notificationservice.service.NotificationService;
import com.fasterxml.jackson.databind.ObjectMapper;
import lombok.RequiredArgsConstructor;
import lombok.extern.slf4j.Slf4j;
import org.springframework.amqp.rabbit.annotation.RabbitListener;
import org.springframework.stereotype.Component;

@Slf4j
@Component
@RequiredArgsConstructor
public class MessageSentEventListener {

    private final NotificationService notificationService;
    private final ObjectMapper objectMapper;

    @RabbitListener(queues = "${app.rabbitmq.message-sent-queue}")
    public void onMessageSent(EventEnvelope<Object> envelope) {
        if (!EventTypes.MESSAGE_SENT.equals(envelope.eventType())) {
            log.warn("Ignored unexpected event type on message queue. eventId={}, eventType={}",
                    envelope.eventId(), envelope.eventType());
            return;
        }

        MessageSentEvent event = toPayload(envelope.payload(), MessageSentEvent.class);
        log.debug("Received MessageSentEvent. eventId={}, messageId={}", envelope.eventId(), event.getMessageId());
        notificationService.createFromMessageSent(event, envelope.eventId());
    }

    private <T> T toPayload(Object payload, Class<T> type) {
        if (type.isInstance(payload)) {
            return type.cast(payload);
        }
        return objectMapper.convertValue(payload, type);
    }
}
