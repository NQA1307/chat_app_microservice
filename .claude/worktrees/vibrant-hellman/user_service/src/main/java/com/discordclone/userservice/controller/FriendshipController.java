...
public class FriendshipController {
    ...
    public ResponseEntity<ApiResponse<FriendResponse>> blockUser(
            @PathVariable("targetUserId") UUID targetUserId,
            HttpServletRequest request) {
        UUID currentUserId = getCurrentUserId(request);
        return ResponseEntity.ok(ApiResponse.success(
                "User blocked successfully",
                friendshipService.blockUser(currentUserId, targetUserId)
        ));
    }
    ...
    @GetMapping("/blocked")
    public ResponseEntity<ApiResponse<List<FriendResponse>>> getBlockedFriends(HttpServletRequest request) {
        UUID currentUserId = getCurrentUserId(request);
        return ResponseEntity.ok(ApiResponse.success(friendshipService.getBlockedFriends(currentUserId)));
    }

    @GetMapping("/{targetUserId}/relationship")
    public ResponseEntity<ApiResponse<com.discordclone.userservice.dto.response.RelationshipResponse>> getRelationship(
            @PathVariable("targetUserId") UUID targetUserId,
            HttpServletRequest request) {
        UUID currentUserId = getCurrentUserId(request);
        return ResponseEntity.ok(ApiResponse.success(friendshipService.getRelationship(currentUserId, targetUserId)));
    }
}
