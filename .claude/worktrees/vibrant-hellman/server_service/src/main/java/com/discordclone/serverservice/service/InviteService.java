...
import com.discordclone.serverservice.repository.ServerBanRepository;
...
public class InviteService {
    ...
    @org.springframework.beans.factory.annotation.Value("${app.rabbitmq.server-invite-sent-routing-key}")
    ...
    public InviteResponse sendServerInvite (
        ...
        if (serverBanRepository.existsByServerIdAndUserId(serverId, request.receiverUserId())) {
            throw new AppException(HttpStatus.FORBIDDEN, "User is banned from this server");
        }
        ...
        eventPublisher.publish(
                EventTypes.SERVER_INVITE_SENT,
                serverInviteSentRoutingKey,
                new ServerInviteEvent(
                        saved.getId(),
                        serverId,
                        senderUserId,
                        request.receiverUserId(),
    ...
    public ServerResponse acceptInvite(String inviteId, UUID receiverUserId) {
        ...
        if (serverBanRepository.existsByServerIdAndUserId(serverId, receiverUserId)) {
